							...
							<form role="form" class="form-inline">
								...
								<div class="form-group mr-20">
									<label for="category" class="mr-5">Category</label> <select
										...
										<option></option>
										<c:forEach items="${ListOfSubCategory}" var="loc">

											<option value="${loc.category.categoryName}">${loc.category.categoryName}</option>
										</c:forEach>
									</select>
								</div>
								<div class="form-group mr-20">
											...
											<div class="btn-group">
												...
												<ul role="menu" class="dropdown-menu drop-dark">
													<c:choose>
														<c:when test="${(loc.active=='0')}">
															<li><a
																href="${pageContext.request.contextPath}/admin/updateSubCategory.io?subCategoryId=${loc.subCategoryId}&amp;active=1">Activate</a></li>
														</c:when>
													</c:choose>
													<c:choose>
														<c:when test="${(loc.active=='1')}">
															<li><a
																href="${pageContext.request.contextPath}/admin/updateSubCategory.io?subCategoryId=${loc.subCategoryId}&amp;active=0">Disable</a></li>
														</c:when>
													</c:choose>
												</ul>
											</div>

				...
				<div class="l-box">
					<div class="l-box-header">
						<h2 class="l-box-title">
							<span>Pending Users</span> Table -- ${status}
						</h2>
					...
					<div class="l-box-body">
						...
						<table id="fooTableId" data-filter="#filter" data-page-size="5"
							...
							<tbody>
								<c:forEach items="${ListOfPendingUsers}" var="loc">
									...
									<tr>
										...
										<td>${loc.time}</td>
										<c:choose>
											<c:when test="${(loc.enabled=='1')}">
												<td data-value="${loc.enabled}"><span title="Active"
													class="label label-success">Active</span></td>
											</c:when>
										</c:choose>
										<c:choose>
											<c:when test="${(loc.enabled=='0')}">
												<td data-value="${loc.enabled+2}"><span
											...
											<div class="btn-group">
												...
												<ul role="menu" class="dropdown-menu drop-dark">
													<c:choose>
														<c:when test="${(loc.enabled=='0')}">
															<li><a
																href="${pageContext.request.contextPath}/admin/updateUsers.io?usersId=${loc.id.username}&amp;active=1&amp;check=ListOfPendingUsers">Activate</a></li>
														</c:when>
													</c:choose>
													<c:choose>
														<c:when test="${(loc.enabled=='1')}">
															<li><a
																href="${pageContext.request.contextPath}/admin/updateUsers.io?usersId=${loc.id.username}&amp;active=0&amp;check=ListOfPendingUsers">Disable</a></li>
														</c:when>
													</c:choose>
												</ul>
											</div>

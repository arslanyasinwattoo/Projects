...
<c:forEach items="${heads}" var="loc">
	<c:choose>
		<c:when
			test="${(loc.users.id.username==loc.messageHeads.usersByFkMessageHeadsUsers1.id.username)}">
			<div class="l-row">
		...
		<c:when
			test="${(loc.users.id.username==loc.messageHeads.usersByFkMessageHeadsUsers2.id.username)}">
			<div class="l-row">

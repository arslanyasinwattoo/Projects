			...
			.bind(
					{
						'footable_filtering' : function(e) {
							...
							if ($('#status').find(':selected').text() == 'Active'
									|| $('#status').find(':selected').text() == 'Disabled') {
								var selected = $('#status').find(':selected')
										.text();

								$('#category').val('');
							} else if ($('#category').find(':selected').text() != "") {

								var selected = $('#category').find(':selected')
										.text();
							} else {

								var selected = $('#subcategory').find(
										':selected').text();
							}
						...
						'footable_filtered' : function() {
							var count = $(fooTableId
									+ ' tbody tr:not(.footable-filtered)').length;
